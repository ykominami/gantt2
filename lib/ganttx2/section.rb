  ...
  class Section
    ...
    def make_section(name, array, prev_end_date, time_span)
      ...
      array.each do |name|
        ...
        itemx = Itemx.new(name, itemx_date, time_span, time_span_unit)
    ...
    def parse_item(item)
      ...
      item.each do |name, time_span|
        # pp "name=#{name}"
        # pp "time_span=#{time_span}"
        # 日付が出現したら
        if name.class == Date
          hashx = { name => time_span }
          parse_under_section(hashx)
        else
          # puts "section.rb:61 parse_item name=#{name} time_span=#{time_span}"
          day, _rest = time_span.split("D")
          if day.length == time_span.length
            day, _rest = time_span.split("d")
            if day.length != time_span.length
              # itemx_date = @prev_itemx.time + @prev_itemx.time_span

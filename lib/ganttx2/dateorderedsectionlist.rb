  ...
  class DateOrderedSectionList
    ...
    def check_error(date, section_name)
      @hash[date][section_name].each do |item|
        raise InvalidClass.new("item must not be Array class") if item.instance_of?(Array)
      end
      raise InvalidClassError.new("it must not be Hash class") if @hash[date][section_name].instance_of?(Hash)
      raise InvalidClassError.new("it must be Array class") unless @hash[date][section_name].instance_of?(Array)
    end
